...
from libs.htx.htx.service.customer import get_customers_from_db, get_customer_by_id
from libs.htx.htx.service.tender_types import get_tender_types_from_db, get_tender_type_by_id
from libs.htx.htx.service.scheda_prompt import get_prompts_from_db, get_prompt_by_id
...
class ChatPage(BasePage):
    def __init__(self, app):
        ...
        # HTX - aggiunta ariabili di stato per costruire il prompt dinamico
        self.customer_selected = gr.State(value=None)
        self.tender_type_selected = gr.State(value=None)
    ...
    def on_building_ui(self):
        with gr.Row():
            ...
            with gr.Column(
                ...
                with gr.Accordion(
                    label="Generated Prompt", open=False, elem_id="generated-prompt", visible=False
                ) as self.generated_prompt:
                    self.generated_prompt_content = gr.HTML(
                        value="",
                        elem_id="generated-prompt-content",
                    )
                with gr.Accordion(
    ...
    def on_register_events(self):
        ...
        # HTX events on TB tab
        self.customer_choice.change(
            fn=self.update_generated_prompt_email,
            inputs=[self.customer_choice, self.tender_type_choice],
            outputs=self.generated_prompt_content
        )

        self.tender_type_choice.change(
            fn=self.update_generated_prompt_email,
            inputs=[self.customer_choice, self.tender_type_choice],
            outputs=self.generated_prompt_content
        )

        self.prompt_choice.change(
            fn=self.update_generated_prompt_scheda,
            inputs=[self.prompt_choice],
            outputs=self.generated_prompt_content
        )
        # HTX - end - events on TB tab
    ...
    # HTX - function that is called when reasoning type is changed
    ...
    # HTX: added function to update the generated prompt
    def update_generated_prompt_email(self, customer_name, tender_type_name):
        customer_id = self.customer_map.get(customer_name, None)  # Trova l'ID del customer
        tender_type_id = self.tender_type_map.get(tender_type_name, None)  # Trova l'ID del tender

        if customer_id and tender_type_id:
            customer = get_customer_by_id(customer_id)
            tender_type = get_tender_type_by_id(tender_type_id)

            customer_description = PromptTemplate(tender_type.customer_description)
            customer_description_prompt = customer_description.populate(
                customer_name=customer.name,
                customer_description=customer.description
            )

            self.email_prompt = f"{tender_type.attachment_description}<br>" \
                                f"{customer_description_prompt}<br>" \
                                f"{tender_type.task_description}<br>" \
                                f"{tender_type.template_description}"
    ...
    def update_generated_prompt_scheda(self, prompt_name):
        ...
        if prompt_id:
            ...
            self.task_prompt = prompt.task
            return gr.update(
                value=f"<div style='white-space: pre-line; word-wrap: break-word;'>{self.task_prompt}</div>")

        return gr.update(value="")

        # HTX: End - added function to update the generated prompt
